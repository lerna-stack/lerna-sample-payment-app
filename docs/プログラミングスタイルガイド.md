...
#### ★ [SHOULD] scala.concurrent.ExecutionContext.Implicits.global は使わず、Akka の dispatcher を使う

`Future` を使った処理を実装していると、Scala コンパイラーに `scala.concurrent.ExecutionContext.Implicits.global` をインポートするように指示されることがありますが（下記参照）、Akka を使っている場合は、Akka の dispatcher を使うことが推奨されます。
...
`scala.concurrent.ExecutionContext.Implicits.global` はアプリケーション全体で使われる可能性があるため、他の部分が原因の（スレッドプールの枯渇といった）不具合の影響を受けやすくなってしまいます。Akka の dispatcher を使うと、処理ごとにスレッドプールを分離したりといったことが設定ファイルの修正だけで実現できるので、そのような不具合の影響を受けにくくすることが簡単にできます。また、パフォーマンスチューニングも容易になります。
...
// OK
// Actor の外側では ActorSystem から dispatcher を取り出す (system は ActorSystem)
import system.dispatcher

// OK
// Actor の内部では context.dispatcher を使う
...
override def receive: Receive = {
  case BadCommand =>
    // NG!
    sender() ! new RuntimeException("illegal command")
  case InvalidCommand =>
    import akka.actor.Status
    // NG!
    sender() ! Status.Failure(new RuntimeException("illegal command"))
  case WrongCommand =>
    // OK
    sender() ! ProcessFailed()
}

sealed trait Response
case class ProcessSucceeded() extends Response
case class ProcessFailed() extends Response
```
...
#### ★ [MUST] Actor から Exception を ! (tell) する場合は Status.Failure で包む

※ Akka Cluster を使う場合など、メッセージのシリアライズが行われるケースでは、そもそも Exception をメッセージに含めることが非推奨です（別項を参照）

メッセージの送信先が `?`（`ask`） している場合、`Exception` を直接返信すると `?` の結果として取得できる `Future` が `Failure[Exception]` ではなく `Success[Exception]` とみなされてしまいます。
処理が成功するか失敗するかによって異なる型のオブジェクトが `Future` に格納されるため、成功した場合の型を期待して `mapTo` すると実行時エラーになります。
...
(actorRef ? BadCommand).mapTo[Ok]       // ClassCastException!
(actorRef ? InvalidCommand).mapTo[Ok]   // Failure[RuntimeException]
(actorRef ? ValidCommand).mapTo[Ok]     // Success[Ok]
```
...
val fizzRegion: ActorRef =
  ClusterSharding(system).start(
    typeName = ClusterShardingTypeNames.fizz,
    entityProps = Props[Fizz],
